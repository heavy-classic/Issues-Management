...
export async function listStages() {
  return db("lesson_workflow_stages").orderBy("position", "asc");
}

export async function getStage(stageId: string) {
  const stage = await db("lesson_workflow_stages").where({ id: stageId }).first();
  if (!stage) throw new AppError(404, "Lesson workflow stage not found");
  return stage;
}
...
export async function createStage(params: CreateStageParams) {
  await db("lesson_workflow_stages")
    .where("position", ">=", params.position)
    .increment("position", 1);

  const [stage] = await db("lesson_workflow_stages")
    .insert({
      name: params.name,
...
export async function updateStage(stageId: string, params: UpdateStageParams) {
  const existing = await db("lesson_workflow_stages").where({ id: stageId }).first();
  if (!existing) throw new AppError(404, "Lesson workflow stage not found");
  ...
  const [updated] = await db("lesson_workflow_stages")
    .where({ id: stageId })
    .update(updateData)
    .returning("*");
...
export async function deleteStage(stageId: string) {
  ...
  if (lessonCount && Number(lessonCount.count) > 0) {
    throw new AppError(
      400,
      "Cannot delete stage: lessons are currently in this stage"
    );
  }

  await db("lesson_workflow_stages").where({ id: stageId }).del();

  const remaining = await db("lesson_workflow_stages").orderBy("position", "asc");
...
export async function reorderStages(stageIds: string[]) {
  const existing = await db("lesson_workflow_stages").select("id");
  ...
  await db("lesson_workflow_stages").update({
